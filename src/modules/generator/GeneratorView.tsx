...
import { Upload, FileText, Sparkles, MessageSquare, Loader2 } from 'lucide-react';
...
export function GeneratorView() {
    const { apiKey, setFlashcards, setIsGenerating, isGenerating } = useStore();
    ...
    const handleCreate = async () => {
      ...
      You must output a JSON object with a key 'flashcards' containing an array of objects. 
      Each object must have 'front' and 'back' fields.
      ...
      Content:
      ${sourceText.slice(0, 15000)} {/* Truncate for safety/token limits for now */}
      `;
            ...
            // Simple cleanup if model returns markdown code block
            const jsonString = response.replace(/^```json/, '').replace(/```$/, '').trim();
            const result = JSON.parse(jsonString);

            if (result.flashcards && Array.isArray(result.flashcards)) {
                const cardsWithIds = result.flashcards.map((card: any) => ({
                    ...card,
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
            ...
            <div className="space-y-6 flex flex-col h-full w-full">
                ...
                {/* Instruction Box */}
                <div className="bg-slate-900/50 backdrop-blur-xl border border-white/10 rounded-2xl p-5 shadow-lg">
                    ...
                    <textarea
                        ...
                        className="w-full bg-slate-950/50 border border-white/10 rounded-xl p-4 text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 resize-none h-32 transition-all"
                    />
                </div>

                {/* Content Input Box */}
                <div className="bg-slate-900/50 backdrop-blur-xl border border-white/10 rounded-2xl p-5 shadow-lg flex-1 flex flex-col min-h-[300px]">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex bg-slate-950/50 rounded-lg p-1 border border-white/5">
                            <button
                                ...
                                className={`px-3 py-1 rounded-md text-sm font-medium transition-all ${activeInputMode === 'text' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}
                            >
                                Text
                            </button>
                            <button
                                onClick={() => setActiveInputMode('file')}
                                className={`px-3 py-1 rounded-md text-sm font-medium transition-all ${activeInputMode === 'file' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}
                            >
                                File
                            </button>
                        </div>
                        ...
                        <AnimatePresence mode="wait">
                            {activeInputMode === 'text' ? (
                                <motion.textarea
                                    ...
                                    className="w-full h-full min-h-[200px] bg-slate-950/50 border border-white/10 rounded-xl p-4 text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-purple-500/50 resize-none"
                                />
                            ) : (
                                <motion.div
                                    key="file-input"
                                    ...
                                    className="w-full h-full min-h-[200px] border-2 border-dashed border-slate-700 hover:border-purple-500/50 bg-slate-950/30 rounded-xl flex flex-col items-center justify-center text-slate-400 transition-colors cursor-pointer relative group"
                                >
                                    <input
                                        type="file"
                                    ...
                                    />
                                    {file ? (
                                        <div className="flex flex-col items-center gap-2 p-4 text-slate-200">
                                            ...
                                            <button
                                                ...
                                                className="mt-2 px-3 py-1 bg-red-500/10 text-red-400 rounded-full hover:bg-red-500/20 text-xs font-bold uppercase tracking-wide z-10"
                                            >
                                                Remove
                                            </button>
                                        </div>
                                        ...
                                        <>
                                            ...
                                            <p className="font-medium text-lg text-slate-300">Click to upload or drag & drop</p>
                ...
                <div className="space-y-2">
                    ...
                    <button
                        ...
                        className="w-full py-4 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white rounded-xl font-bold text-lg shadow-lg shadow-blue-900/20 hover:shadow-blue-900/40 transform hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed group"
                    >
                        {isGenerating ? (
                            <>
            ...
            <div className="bg-slate-900/50 backdrop-blur-xl border border-white/10 rounded-2xl p-8 flex items-center justify-center relative overflow-hidden shadow-2xl flex-col h-full min-h-[500px]">
                ...
                {generatedPreview.length > 0 ? (
                    <div className="w-full max-w-md relative z-10 scale-90 lg:scale-100 transition-transform h-full flex flex-col">
                        ...
                        <div className="mt-8 text-center bg-slate-950/50 p-4 rounded-xl border border-white/5 backdrop-blur-md">
